...
const appsvcPath = process.env.appsvcPath;
...
function buildRestPayload(eventJson) {
    ...
    try {
        instanceId = eventJson["resources"][0].split('/').slice(-1)[0];
        let msgTypeStr = eventJson["detail-type"];
        if (msgTypeStr === "EC2 Instance State-change Notification") {
            msgTypeValue = 11;
        } else if (msgTypeStr === "Tag Change on Resource") {
            msgTypeValue = 12;
        }
    } catch (e) {
    ...
    return {key: instanceId, msg: {MsgType: msgTypeValue, MsgOp: 3, MsgBody: eventJson}};
}
...
async function postMsgToEsaas(eventStr) {
    ...
    const options = {
        host: apiGwHost,
        port: apiGwPort,
        path: `${appsvcPath}?topic=${kafkaTopic}&key=${payloadJson.key}`,
        method: 'POST',
